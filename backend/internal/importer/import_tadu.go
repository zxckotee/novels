...
// ImportTaduResumable is a pause/resume-friendly version of ImportTadu:
// - Creates novel+localizations once (checkpoint.NovelID).
// - Imports chapters in per-chapter transactions.
// - Updates checkpoint.NextIndex after each saved chapter.
//
// Cancellation is cooperative via ctx; caller should persist checkpoint after each onChapter callback.
func ImportTaduResumable(
	ctx context.Context,
	db *sqlx.DB,
	opts ImportTaduOptions,
	checkpoint *Checkpoint,
	onChapter func(cp *Checkpoint, chaptersSaved int) error,
) (*ImportTaduResult, *Checkpoint, error) {
	if db == nil {
	...
	// Create novel once (or reuse)
	if checkpoint.NovelID == uuid.Nil {
		novelID := uuid.New()
		novelSlug := generateSlugTadu(book.Title, novelID)

		locs := []models.CreateLocalizationRequest{
			{Lang: "ru", Title: book.Title, Description: strPtrOrNilOrParserTadu(book.Description), AltTitles: []string{"parser"}},
			{Lang: "en", Title: book.Title, Description: strPtrOrNilOrParserTadu(book.Description), AltTitles: []string{"parser"}},
			{Lang: "zh", Title: book.Title, Description: strPtrOrNilOrParserTadu(book.Description), AltTitles: []string{"parser"}},
			{Lang: "ja", Title: book.Title, Description: strPtrOrNilOrParserTadu(book.Description), AltTitles: []string{"parser"}},
			{Lang: "ko", Title: book.Title, Description: strPtrOrNilOrParserTadu(book.Description), AltTitles: []string{"parser"}},
			{Lang: "fr", Title: book.Title, Description: strPtrOrNilOrParserTadu(book.Description), AltTitles: []string{"parser"}},
			{Lang: "de", Title: book.Title, Description: strPtrOrNilOrParserTadu(book.Description), AltTitles: []string{"parser"}},
		}
		...
		if strings.HasPrefix(book.CoverURL, "http") {
			key, err := downloadAndStoreCoverTadu(ctx, novelID, book.CoverURL, opts.UploadDir)
			if err == nil && key != "" {
		...
		// return coverKey via result later
		_ = coverKey
	}
...
func ImportTadu(ctx context.Context, db *sqlx.DB, opts ImportTaduOptions) (*ImportTaduResult, error) {
	if db == nil {
...
func generateSlugTadu(title string, novelID uuid.UUID) string {
...
func strPtrOrNilOrParserTadu(s string) *string {
...
func downloadAndStoreCoverTadu(ctx context.Context, novelID uuid.UUID, coverURL string, uploadDir string) (string, error) {
