...
type Import69ShubaOptions struct {
	PageURL          string
...
type Import69ShubaResult struct {
	NovelID       uuid.UUID
	Slug          string
	ChaptersTotal int
	ChaptersSaved int
...
func Import69ShubaResumable(
	ctx context.Context,
	db *sqlx.DB,
	opts Import69ShubaOptions,
	checkpoint *Checkpoint,
	onChapter func(cp *Checkpoint, chaptersSaved int) error,
) (*Import69ShubaResult, *Checkpoint, error) {
	if db == nil {
	...
	if checkpoint.NovelID == uuid.Nil {
		novelID := uuid.New()
		novelSlug := generateSlug69(book.Title, novelID)

		locs := []models.CreateLocalizationRequest{
			{Lang: "ru", Title: book.Title, Description: strPtrOrNilOrParser69(book.Description), AltTitles: []string{"parser"}},
			{Lang: "en", Title: book.Title, Description: strPtrOrNilOrParser69(book.Description), AltTitles: []string{"parser"}},
			{Lang: "zh", Title: book.Title, Description: strPtrOrNilOrParser69(book.Description), AltTitles: []string{"parser"}},
			{Lang: "ja", Title: book.Title, Description: strPtrOrNilOrParser69(book.Description), AltTitles: []string{"parser"}},
			{Lang: "ko", Title: book.Title, Description: strPtrOrNilOrParser69(book.Description), AltTitles: []string{"parser"}},
			{Lang: "fr", Title: book.Title, Description: strPtrOrNilOrParser69(book.Description), AltTitles: []string{"parser"}},
			{Lang: "de", Title: book.Title, Description: strPtrOrNilOrParser69(book.Description), AltTitles: []string{"parser"}},
		}
		...
		if strings.HasPrefix(book.CoverURL, "http") {
			key, err := downloadAndStoreCover69(ctx, novelID, book.CoverURL, opts.UploadDir)
			if err == nil && key != "" {
...
func Import69Shuba(ctx context.Context, db *sqlx.DB, opts Import69ShubaOptions) (*Import69ShubaResult, error) {
	if db == nil {
	...
	// use source language for all localizations for now (ru required by UI)
	locs := []models.CreateLocalizationRequest{
		{Lang: "ru", Title: book.Title, Description: strPtrOrNilOrParser69(book.Description), AltTitles: []string{"parser"}},
		{Lang: "en", Title: book.Title, Description: strPtrOrNilOrParser69(book.Description), AltTitles: []string{"parser"}},
		{Lang: "zh", Title: book.Title, Description: strPtrOrNilOrParser69(book.Description), AltTitles: []string{"parser"}},
	...
	var coverKey *string
	if strings.HasPrefix(book.CoverURL, "http") {
		// cover download is best-effort; keep existing behavior but without site fetcher
		key, err := downloadAndStoreCover69(ctx, novelID, book.CoverURL, opts.UploadDir)
		if err == nil && key != "" {
			_, err = tx.ExecContext(ctx, `UPDATE novels SET cover_image_key = $1 WHERE id = $2`, key, novelID)
			if err != nil {
	...
	for i, ref := range chRefs {
		...
		chapterID := uuid.New()
		// Prefer site-provided numbering from catalog (li[data-num]) if available.
		number := float64(i + 1)
		if ref.Number != nil && *ref.Number > 0 {
			number = float64(*ref.Number)
		}
...
func generateSlug69(title string, novelID uuid.UUID) string {
...
func strPtrOrNilOrParser69(s string) *string {
...
func downloadAndStoreCover69(ctx context.Context, novelID uuid.UUID, coverURL string, uploadDir string) (string, error) {
